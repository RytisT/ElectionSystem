...
public class District_Representatives {
    ...
    @NotNull
    @Pattern(regexp = ".*([a-zA-Z0-9ąčęėįšųūžĄČĘĖĮŠŲŪŽ„“]$)", message = "District Representative LOGIN contains invalid characters. ")
    @Length(min = 1, max = 40, message = "District Representative LOGIN must not be empty and length can not be longer than {max} symbols. ")
    private String login;

    @Column(name = "password")
    @NotNull
    @Pattern(regexp = ".*([a-zA-Z0-9ąčęėįšųūžĄČĘĖĮŠŲŪŽ„“]$)", message = "District Representative PASSWORD contains invalid characters. ")
    @Length(min = 1, max = 30, message = "District Representative PASSWORD must not be empty and length can not be longer than {max} symbols. ")
